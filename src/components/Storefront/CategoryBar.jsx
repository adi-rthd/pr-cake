...
const CategoryBar = () => {
  ...
  return (
    <div className="w-full bg-[#FAF7F2] py-12 border-b border-[#E6DFD3] overflow-hidden">
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex overflow-x-auto pb-6 scrollbar-hide -mx-6 px-6 md:mx-0 md:px-0 gap-6 md:gap-8 justify-start xl:justify-center items-center">
          {categories.map((cat) => (
            <div key={cat.id} className="flex flex-col items-center gap-3 cursor-pointer group shrink-0">
              <div className="w-20 h-20 md:w-24 md:h-24 rounded-full p-1 border-2 border-transparent group-hover:border-brand-rose transition-colors duration-300">
                <div className="w-full aspect-square rounded-full overflow-hidden shadow-sm bg-white relative">
                  <img 
                    src={cat.image} 
                    alt={cat.name} 
          ...
          {/* View All */}
          <div className="flex flex-col items-center gap-3 cursor-pointer group shrink-0 ml-4 md:ml-0">
            <div className="w-20 h-20 md:w-24 md:h-24 rounded-full p-1 border-2 border-transparent group-hover:border-brand-brown transition-colors duration-300">
              <div className="w-full aspect-square rounded-full bg-white border border-[#E6DFD3] flex items-center justify-center shadow-sm relative">
                <span className="text-2xl text-stone-400 group-hover:text-brand-brown transition-colors">→</span>
              </div>
            </div>
            <span className="text-xs md:text-sm font-semibold text-stone-600 group-hover:text-brand-brown transition-colors">
              {t('viewAll') || 'View All'}
